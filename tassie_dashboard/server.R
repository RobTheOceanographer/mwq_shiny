...
#     if (input$dataType == 'chl'){
#       legend_img <- readPNG("CHL_chlor_a_colorscale.png")  
#     }
#     if (input$dataType == 'sst'){
#       legend_img <- readPNG("SST_colorscale.png")  
#     }
    legend_img <- readPNG("CHL_chlor_a_colorscale.png")  
  ...
  output$mymap <- renderLeaflet({
    ...
    #if (input$dataType == 'chl'){
      usr_base_url = "/Users/rjohnson/Documents/2.CURRENT_PROJECTS/mwq_shiny/tassie_dashboard/local_data_store/chl"
      usr_grid_url = paste(usr_base_url, paste('A', usr_year,usr_month,usr_day,".70.aust.chl_oc3.grd",sep=""), sep="/")
      chl_data <- raster(usr_grid_url)
      leaflet() %>% addTiles() %>% setView(147, -42, zoom = 6) %>% addRasterImage(chl_data, color=chlpalfun , project= TRUE,opacity = 0.8, attribution = HTML(paste(a('NASA MODIS-Aqua Ocean Colour OCv3 Chlorophyll Product',target='_blank',href='http://oceancolor.gsfc.nasa.gov/cgi/browse.pl?sen=am')))) 
    #}
#     if (input$dataType == 'sst'){
