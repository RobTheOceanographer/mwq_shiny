...
add_offset <- ncatt_get(f, var_string)$'add_offset'
scale_factor <- ncatt_get(f, var_string)$'scale_factor'
...
leaflet() %>% addTiles() %>% setView(150, -40, zoom = 5) %>% addRasterImage(r_f.cel.flipped.cropped, color= sstpalfun, project= TRUE,opacity = 0.8, attribution = HTML(paste(a('eReesf CHL',target='_blank',href='http://ereeftds.bom.gov.au/ereefs/tds/catalog/ereef/mwq/catalog.html'), icon('copyright'))))



writeRaster(r_f.cel.flipped.cropped, filename = "/Users/rjohnson/Documents/2.CURRENT_PROJECTS/mwq_shiny/tassie_dashboard/r_f.cel.flipped.cropped")
