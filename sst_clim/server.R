...
####### Observe user clicks. #######
  observe({#Observer to show Popups on click
    ...
    if (!is.null(click)) {
      showpos(x=click$lng, y=click$lat,r_f)
    }
  })
...
####### chlorophyll data with Leaflet #######
  output$mymap <- renderLeaflet({
    ...
    usr_grid_url = paste(usr_base_url, paste('c',enso, month ,"ave_clim.nc",sep="_"), sep="")
    ...
    #pal <- colorNumeric(c("#fff7ec", "#fee8c8","#fdd49e","#fdbb84","#fc8d59","#ef6548","#d7301f","#b30000","#7f0000"), values(r_f), na.color = "transparent")
    #pal <- colorNumeric(c("#0C2C84", "#41B6C4", "#FFFFCC"), values(r_f), na.color = "transparent")
    pal <- colorNumeric(palette = sst.pal(palette = TRUE)$cols, sst.pal(palette = TRUE)$breaks, na.color = "transparent")
    
    ## This is the chlorophyll leaflet itself ##
    #leaflet() %>% addTiles() %>% setView(147, -42, zoom = 8) %>% addRasterImage(r_f, colors = pal, opacity = 0.8, attribution = HTML(paste(a('GHRSST Bouy Depth SST',target='_blank', href='http:'),icon('copyright'),'Produced by the Me.')),)  %>% addLegend(pal = pal, values = values(r_f), title = "Temp (Celcius)")
    leaflet() %>% addTiles() %>% setView(147, -42, zoom = 8) %>% addRasterImage(r_f, colors = sstpalfun, opacity = 0.8, attribution = HTML(paste(a('IMOS GHRSST Bouy Depth SST',target='_blank', href='http://thredds.aodn.org.au/thredds/catalog/IMOS/SRS/SST/ghrsst/L3S-1d/ngt/catalog.html'))),)  %>% addLegend(pal = pal, values = sst.pal(palette = TRUE)$breaks, title = "Temp (Celcius)", opacity=1)
    
    # ## This is the chlorophyll leaflet itself ##
    # leaflet() %>% addTiles() %>% setView(147, -42, zoom = 8) %>% addRasterImage(r_f,  colors = chlpalfun, opacity = 0.8, attribution = HTML(paste(a('GHRSST Bouy Depth SST',target='_blank',
    #                                                                                                                                                 href='http:'),
    #                                                                                                                                               icon('copyright'),
    #                                                                                                                                               'Produced by the Me.')),)
