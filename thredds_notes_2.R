...
m <- leaflet() %>%
  ...
  # %>%
  # addLegend(pal = pal, values = values(r_f), title = var_string)
  #addLegend(pal = chlpalfun, values = values(r_f) )
  #addLegend(position = 'bottomright', colors = rgb(t(col2rgb(chlpalfun(c(0.1, 0.03, 0.5, 0.1, 0.3, 1, 3, 10 )))) / 255), labels = c("0.1", "0.03", "0.5", "0.1", "0.3", "1", "3", "10"), opacity = 1,title = var_string)
